...
class ChessNetConservative(nn.Module):
    ...
    Input:  [B, 18, 8, 8]
    Output: policy_logits [B, 4096], value [B, 1] (raw eval in pawns)
    """
    def __init__(self, in_ch=18, width=192, n_blocks=12, drop_path_max=0.1):
        ...
        # Value head (NO tanh, raw pawn eval)
...
class ChessPositions(Dataset):
    ...
    def __init__(self, data_dir, batch_ids):
        ...
        self.X = np.concatenate(X_list, axis=0)          # (M,18,8,8), float32
        self.y_pol = np.concatenate(y_pol_list, axis=0)  # (M,4096), float32
        self.y_val = np.concatenate(y_val_list, axis=0)  # (M,), float32

        # Precompute class index for policy
        self.move_idx = self.y_pol.argmax(axis=1).astype(np.int64)  # (M,)
...
def discover_batch_ids(data_dir):
    """
    Find all X_boards_batch_*.npy files in data_dir and return
    their numeric batch indices as a sorted list.
    """
...
def main():
    ...
    num_epochs = 5
    value_clip = 1.0  # clip evals to [-8, 8] pawns for training stability
    ...
    for epoch in range(num_epochs):
        ...
        for b in batch_ids:
            print(f"  → Training on batch {b:04d} ...", end="", flush=True)
            ...
            for x, move_idx, value in loader:
                ...
                # Clip labels to [-value_clip, value_clip]
                ...
                # Optionally clip predictions too
        ...
        ckpt_path = f"chessnet_mvp_18planes_epoch{epoch+1}.pt"
        torch.save(model.state_dict(), ckpt_path)
        print(f"Saved checkpoint: {ckpt_path}")

    # Final save
    final_path = "chessnet_mvp_18planes_final.pt"
